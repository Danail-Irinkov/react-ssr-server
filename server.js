...
import app_raw from './src/App.js'
// import mainCss from './src/assets/scss/main.css';
let mainCss = fs.readFileSync('./src/assets/scss/main.css', 'utf-8').toString()

let body = DOM.body, div = DOM.div, script = DOM.script, head = DOM.head
// This is our React component, shared by server and browser thanks to browserify
// let App = React.createFactory(import('./App.js'))
// let app_raw = fs.readFileSync('./src/App.js', 'utf-8').toString()
// console.log('app_raw', app_raw)
// debugger
...
http.createServer(function(req, res) {
  ...
  if (req.url === '/') {
    ...
    // language (or just a string) for the outer page template
	  let App = React.createElement(app_raw, props)
    let html = ReactDOMServer.renderToStaticMarkup(
			DOM.html({},
	    head(null, DOM.link({type: 'text/css'}), DOM.title({}, 'asdad'),
		    DOM.style({},
		    ".header-nav-toggle {width: 200px}",
			    mainCss
		    )),
			body(null,
      ...
      div({
        id: 'content',
        dangerouslySetInnerHTML: {__html: ReactDOMServer.renderToString(App)},
      }),
